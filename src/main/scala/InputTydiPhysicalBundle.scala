...
class InputTydiPhysicalBundle(laneCounts: InputStreamsAsFields[Int]) extends Bundle with InputStreamsAsFields[PhysicalStream] {
  private val char = BitsEl(8.W)
  val posts: PhysicalStream = PhysicalStream(new PostStreamGroup, laneCounts.posts, 1, 1)
