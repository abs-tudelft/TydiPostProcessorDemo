...
class OutputTydiPhysicalBundle extends Bundle with InputStreamsAsFields[PhysicalStream] {
  private val char = BitsEl(8.W)
  val posts: PhysicalStream = PhysicalStream(new PostWithMetadata, 1, 1, 1)
